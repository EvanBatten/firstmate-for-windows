...
test_full_scan_budget_includes_wake_lock_wait() {
  ...
  elapsed=$(( $(date +%s) - started ))
  reap "$holder"
  # The scan gives up the wake-lock wait at its own deadline, and the
  # process-group backstop covers a host too slow to reach it; the bound proves
  # the scan cannot ride the 30-second lock hold.
...
scan_lock_entries() { # <state>
  local entry found=''
  for entry in "$1"/.inactive-outcome-reconcile.lock*; do
...
# Hold MAIN's wake-queue lock from another process until release_wake_queue_lock,
# which lets the holder release it cleanly so a later scan has nothing to steal.
hold_wake_queue_lock() {
  ...
  FM_HOME="$MAIN" FM_STATE_OVERRIDE="$MAIN/state" bash -c '
    ...
    : > "$2/lock-ready"
    i=0
    while [ ! -e "$2/lock-release" ] && [ "$i" -lt 900 ]; do sleep 0.1; i=$((i + 1)); done
...
test_backstop_kill_leaves_no_scan_lock_behind() {
  ...
  FM_INACTIVE_RECONCILE_BUDGET_SECS=$budget FM_FAKE_CREW_STATE='done' run_reconcile "$MAIN" --startup
  leftovers=$(scan_lock_entries "$MAIN/state")
  [ -z "$leftovers" ] || fail "the following scan left lock entries: $leftovers"
  pass "a bounded scan leaves no lock behind"
}
...
test_stale_scan_lock_recovery_leaves_no_steal_behind() {
  ...
  ' _ "$ROOT" "$lock" || fail "could not seed a scan lock"
  seeded=$(readlink "$lock" 2>/dev/null) || fail "the seeded scan lock is not a link"
  [ -n "$(cat "$seeded/pid" 2>/dev/null)" ] || fail "the seeded scan lock names no owner pid"
  hold_wake_queue_lock

  FM_INACTIVE_RECONCILE_BUDGET_SECS=$budget FM_FAKE_CREW_STATE='done' run_reconcile "$MAIN" --startup &
  scan=$!
  while kill -0 "$scan" 2>/dev/null; do
    if current=$(readlink "$lock" 2>/dev/null) && [ -n "$current" ] && [ "$current" != "$seeded" ]; then
      recovered=1
      break
    fi
    sleep 0.1
  done
  [ "$recovered" -eq 1 ] || fail "the scan never recovered the stale lock before its bound ended it (budget ${budget}s is too small for this host)"
  wait "$scan" || true
  release_wake_queue_lock
  leftovers=$(scan_lock_entries "$MAIN/state")
