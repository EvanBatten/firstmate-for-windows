...
fm_private_mode_ok "$H/state/when/when-arm-test.spec" 600 \
  || fail "the armed spec is not mode 0600: $(fm_private_stat_mode "$H/state/when/when-arm-test.spec")"
...
for _ in $(seq 1 20); do
  descendant_state=$(fm_test_stat "$descendant_pid" | tr -d ' ' || true)
