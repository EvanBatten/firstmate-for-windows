...
fm_backend_source cmux || { echo "skip: could not source the cmux adapter"; exit 77; }

fm_backend_cmux_tool_check >/dev/null 2>&1 || { echo "skip: cmux CLI not found on PATH or at the bundle path"; exit 77; }
fm_backend_cmux_version_check >/dev/null 2>&1 || { echo "skip: installed cmux is older than the verified minimum"; exit 77; }
PING_STATE=$(fm_backend_cmux_ping_state)
[ "$PING_STATE" = ok ] || { echo "skip: cmux socket not reachable/authenticated (state=$PING_STATE) - see docs/cmux-backend.md 'Setup'"; exit 77; }
