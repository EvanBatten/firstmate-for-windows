...
test_branch_default_on_heartbeat_afk_and_fallback() {
  ...
  install_pi_branch_extension_fixture "$repo"
  cp "$ROOT/bin/fm-lease.sh" "$ROOT/bin/fm-lease-lib.sh" "$ROOT/bin/fm-private-lib.sh" "$ROOT/bin/fm-wake-lib.sh" \
    "$ROOT/bin/fm-proc-lib.sh" "$ROOT/bin/fm-wake-grant.sh" "$broken/bin/"
