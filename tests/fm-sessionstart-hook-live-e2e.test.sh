...
make_lab() {  # <harness> -> echoes lab dir
  ...
  # The REAL deferred-network stage plus the libraries it sources, directly or
  # through fm-wake-lib.sh, so fact (c) is proven against the actual detach
  # this ship relies on rather than a re-creation of it. Its bootstrap child is
  # a stub: what is under test here is survival across the hook boundary, not
  # the sweeps, which tests/fm-bootstrap.test.sh already owns.
