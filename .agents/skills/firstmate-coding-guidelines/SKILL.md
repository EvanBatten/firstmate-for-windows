...
- Run `bin/fm-check.sh` before treating a change as done; it runs shell syntax, the documentation inventory, and then `bin/fm-lint.sh`, which is the single owner of the lint definition (file set, config, pinned shellcheck version, and pinned actionlint workflow lint) that CI and the no-mistakes pre-push gate both invoke, and it refuses to run under any other version of either linter.
- Before reporting a change to what a `bin/` script does as done, load `verify-firstmate` and prove the affected feature for real, once on the base and once on the change.
