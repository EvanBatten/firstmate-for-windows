...
One condition overrides every row that would spend a digest: an MSYS userland (Git Bash, MSYS2, Cygwin) whose ancestry names no harness.
...
So `bin/fm-sessionstart-run.sh` hands the open to the nudge tier in that case and spends one instruction line instead of a bounded digest that cannot take the helm.
Only the digest-running sources ask the question: `resume`, `reload` and `fork` reach the same nudge whichever way it is answered, and the walk costs about a second on that platform, so they delegate straight to it and the nudge does its own single walk.
The severed ancestry, not the userland, is the gate, because the tracked registrations are written to keep it: the Claude and Cursor entries both end in `; exit 0`, so the wrapper is never the shell's final command, and the Codex entry pipes its payload into the wrapper, which bash forks as a pipeline element rather than exec-optimizing.
No correct shell may fold the `; exit 0` form away, because exec-ing the wrapper would hand the harness the wrapper's own exit status while the builtin pins the hook's status to 0, which is the exit contract both wrappers already owe every transport.
All three transports therefore keep a live parent chain the hybrid walk crosses to the native harness.
...
What still reaches the divert is a registration that execs the wrapper - an operator's own, or an older copy of a tracked one - a host with no `pwsh`, where the Win32 half of the walk cannot run at all, and a native parent the classifier cannot name, which is Pi on Windows, whose Win32 parent is `node.exe`.
When the wrapper does divert it passes `--no-harness-ancestry` to the nudge, because it has just established that no harness is in this process's ancestry and therefore that no lock can be proven this session's, so the open pays for one walk instead of two.
The flag is an argument rather than an environment variable so nothing the nudge or the caller later spawns can inherit it.
...
Where the ancestry is severed the routing inverts and every source reaches the nudge, which is the one case in which it fires for a `clear` or a `compact`.
...
When the run wrapper diverts an open it passes `--no-harness-ancestry`, and the nudge then skips that check entirely rather than repeating the walk the caller just paid for along with the live-table scan its liveness probe needs first.
...
| --- | --- | --- | --- |
| Claude | Run | `.claude/settings.json` registers one unmatched `SessionStart` hook, invoked through `CLAUDE_PROJECT_DIR` with a 180s timeout; the wrapper reads `source` from the hook payload. The registration ends in `; exit 0` rather than exec-ing the wrapper, so the wrapper is forked, keeps a live parent, and cannot hand the harness its own exit status. | Native stdout context injection is supported. |
...
It runs the tracked Claude `SessionStart` command string the way the harness does, against a stub that records its own parent and exits non-zero, so a registration that exec'd the wrapper away is caught on the process tree rather than inferred from the JSON bytes, and the hook's pinned exit 0 is proven on the same run.
A further case pins the other end of that same chain on the process tree: a `resume` open reaches the nudge as the child of a live wrapper rather than through `exec`, so the nudge's ownership check still has this session's ancestry to walk, and a nudge that exits 2 there leaves the hook's status 0 all the same.
Two more of its cases pin that the run wrapper resolves an open's ancestry once: a `compact` open on an MSYS transport whose ancestry does resolve keeps its `--reemit` and starts exactly one ancestry walk before the digest runs, against a stubbed digest because the real one walks again inside `bin/fm-lock.sh`, and a diverted MSYS open with a live foreign lock prints exactly the nudge line while walking once and scanning the live process table not at all.
`tests/fm-proc-lib.test.sh` proves the prime those two rest on: a prime for a pid the memo already describes starts no second `pwsh`, a prime for a pid it does not still walks, and an `FM_PROC_CHAIN_MEMO` arriving from the environment is never read.
...
It runs the tracked `sessionStart` command string the same way the Claude case above does, so the registration is proven to leave the adapter a live parent instead of exec-ing it away.
