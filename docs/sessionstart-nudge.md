...
The severed ancestry, not the userland, is the gate, because the tracked registrations are written to keep it: the Claude and Cursor entries both end in `; exit 0`, so the wrapper is never the shell's final command, and the Codex entry pipes its payload into the wrapper, which forks a pipeline element too.
