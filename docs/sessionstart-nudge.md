...
Two more of its cases pin that one session open resolves its ancestry once: a `compact` open on an MSYS transport whose ancestry does resolve keeps its `--reemit` and starts exactly one ancestry walk for the whole open, and a diverted MSYS open with a live foreign lock prints exactly the nudge line while walking once and scanning the live process table not at all.
