...
The Claude mode waits a cooperation window and allows the stop when the watcher is healthy, the auto-arm's generation claim is open, or `state/.claude-autoarm-epoch` contains a fresh actionable rewake owned by this event epoch.
That window is a wall-clock deadline rather than a count of polls, because a poll that costs hundreds of milliseconds silently shrinks a counted window on a slow host.
It is at least `FM_CLAUDE_AUTOARM_SYNC_WAIT_MS` (default 800 milliseconds), widened to twice the host's slowest measured time-to-claim, and bounded by `FM_CLAUDE_AUTOARM_SYNC_WAIT_MAX_MS` (default 120 seconds).
The measurement is `state/.claude-autoarm-claim-ms`, one integer of milliseconds that the auto-arm writes when, and only when, it claims a home and beats the value already there, so the record holds the home's slowest observed time-to-claim rather than its most recent.
It will not record a measurement above `FM_CLAUDE_AUTOARM_CLAIM_MS_MAX` (default 60 seconds), because the measurement is a wall-clock delta and a claim that slow is a suspend or a clock step rather than this host's speed, which a record that only rises would otherwise keep forever.
A malformed record leaves the window at `FM_CLAUDE_AUTOARM_SYNC_WAIT_MS`, which is what a host that claims in milliseconds keeps.
A home that has never claimed, with no record and no epoch ledger yet, waits `FM_CLAUDE_AUTOARM_FIRST_CLAIM_WAIT_MS` (default 15 seconds, 90 seconds on Git Bash) on that first turn end, because its first claim is the slow one and a forced continuation there is not one turn but the rest of the session, since the primary then polls its workers instead of ending a turn again (#86).
A home that has claimed before but holds a malformed record still blocks after its configured window rather than after whatever the poll happened to cost, which on a slow host is less wall clock than the counted loop accidentally spent.
A missing or malformed cap is the other case and behaves differently on purpose: it falls back to the default 120 seconds and a recorded measurement still widens the window, because a typo in the cap must not silently reintroduce the forced continuation the widening exists to remove.
