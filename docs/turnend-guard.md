...
The measurement is `state/.claude-autoarm-claim-ms`, one integer of milliseconds that the auto-arm writes when, and only when, it claims a home and beats the value already there, so the record holds the home's slowest observed time-to-claim rather than its most recent; a missing or malformed record leaves the window at `FM_CLAUDE_AUTOARM_SYNC_WAIT_MS`, which is what a host that claims in milliseconds keeps.
