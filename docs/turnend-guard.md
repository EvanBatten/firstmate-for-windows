...
It is at least `FM_CLAUDE_AUTOARM_SYNC_WAIT_MS` (default 800 milliseconds), widened to twice the host's last measured time-to-claim, and bounded by `FM_CLAUDE_AUTOARM_SYNC_WAIT_MAX_MS` (default 15 seconds).
The measurement is `state/.claude-autoarm-claim-ms`, one integer of milliseconds that the auto-arm writes when, and only when, it claims a home; a missing or malformed record leaves the window at `FM_CLAUDE_AUTOARM_SYNC_WAIT_MS`, which is what a host that claims in milliseconds keeps.
