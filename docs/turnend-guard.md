...
It is at least `FM_CLAUDE_AUTOARM_SYNC_WAIT_MS` (default 800 milliseconds), widened to twice the host's slowest measured time-to-claim, and bounded by `FM_CLAUDE_AUTOARM_SYNC_WAIT_MAX_MS` (default 15 seconds).
...
A missing or malformed cap is the other case and behaves differently on purpose: it falls back to the default 15 seconds and a recorded measurement still widens the window, because a typo in the cap must not silently reintroduce the forced continuation the widening exists to remove.
