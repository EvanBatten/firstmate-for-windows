...
A missing or malformed record leaves the window at `FM_CLAUDE_AUTOARM_SYNC_WAIT_MS`, which is what a host that claims in milliseconds keeps.
That is the same configured window as the counted loop had, but on a slow host it is less wall clock than the count accidentally spent, so a home that has never claimed now blocks after its configured window rather than after whatever the poll happened to cost.
