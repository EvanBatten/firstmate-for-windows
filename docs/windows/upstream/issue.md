...
On a filesystem that carries modes each helper is the expression it replaced, at the same cost: the setter reaches the probe only when its `chmod` fails and the assertion only when its comparison disagrees, so a genuinely group-readable file is refused there exactly as before.
