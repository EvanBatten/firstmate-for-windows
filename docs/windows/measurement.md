...
Two fixture timing budgets surfaced behind the fixed mode gate and were sized to the host rather than enlarged, following issue #8's rule: five fixed half-second waits, and a sibling process that had to outlive a setup slower than its own fixed thirty seconds.
