...
| #2 | 22 | Opus | `bin/fm-sessionstart-run.sh` hands the open to the nudge tier on an MSYS userland, whatever the source, instead of a digest that cannot take the lock | two cases (MSYS nudges, POSIX still runs the digest), each green in isolation; the suite still stops at its known OpenCode case 8 |
| #5 | 25 | Opus | `fm_pid_identity` records `btime + field22/CLK_TCK` on a non-Linux `/proc` under its own `proc-createtime` key, `btime` read without a fork and `CLK_TCK` read once | a fake `/proc` stepped by the measured 1.041 s yields the same identity; 20 reads of a live pid over 45 s, one string; six cases green |
