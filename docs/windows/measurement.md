...
| 25 | Watcher identity across a clock step | **FIXED** (issue #5, in the issue sweep) | `fm_pid_identity`'s `proc-starttime` is `/proc/<pid>/stat` field 22, and its comment says that field is "immune to the wall-clock steps". True on Linux, false here: `/proc/stat`'s `btime` is `now - uptime` RECOMPUTED at every read (measured: `1788065531 - 976306 = 1787089225` against a reported `1787089224`), and field 22 is anchored to that derived origin (`btime + field22/CLK_TCK = 1788065561` against Win32's true creation time `1788065562`). `CLK_TCK` here is **1000**, not 100, so Phase C's 1041-tick drift is 1.04 s of clock adjustment, not 10.4 - an ordinary NTP resync evicts a live watcher and a closed lid is not required. Both values stable to the tick over 25 s with no step. The sweep built the recommendation this row made, in a corrected form the review of that PR forced: `btime + field22/CLK_TCK` is invariant only under a WHOLE-second step, because `btime` is truncated to seconds while field 22 carries milliseconds, so a fractional correction - the ordinary NTP shape - still moves the sum by a second. What shipped is the same origin at full precision, `now - uptime + field22/CLK_TCK` floored once, where `now` is bash's `EPOCHREALTIME` and `uptime` is monotonic. The residual is /proc/uptime's centisecond granularity plus the gap between the two clock reads, and it is persistent per process rather than random per read: a process whose creation instant lands within about 15 ms of a second boundary can be recorded one second apart by two readers for its whole life, so every one of its health checks is a coin toss rather than one unlucky read. Only a tolerance at the roughly fifteen sites that compare these strings for equality, or Win32's own creation time, removes it. `bin/fm-teardown.sh`'s `task_process_identity` is a second reader of field 22 that still records the raw tick count, left alone here because it captures and re-compares within one reap loop and the whole path is gated on `lsof`, which this port does not have, so a step there means "do not signal" rather than signalling the wrong pid. | slice 10 measured it and changed no code; the sweep built it |
...
Stacked on `fm/issue-sweep-1` and re-verified there after a rebase onto `windows`: `fm-jq-lib` 7/7, `fm-lint-workflows` 18/18, `fm-backend-cmux` 53 then #14, `fm-watcher-lock` 17 then the documented flaky grace case (issue #8), `fm-sessionstart-nudge` 7 then case 8, lint and whitespace clean.
This means every red left on the stack is one the ledger already names, and every new case runs green on this box.
