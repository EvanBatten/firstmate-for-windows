...
It does not get a probe of its own - that would be a second answer to a question this owner exists to answer once - so `bin/fm-procevent.sh` asks the owner and passes the verdict down as `--private-modes enforcing|unenforceable`, the helper treats anything else as the strict mode, and only the mode equality relaxes: ownership, symlink, type, link-count, size and `O_CREAT|O_EXCL|O_NOFOLLOW` all keep refusing, and files are still created asking for 0600.
`bin/fm-extension.mjs` carries the same exact-mode assertions on the same capture path and is untouched here, so that half of the path still refuses on such a mount; it is a separate trade, because the node host also asserts the 0555/0444 modes of an installed package tree, which is an immutability claim rather than a privacy one.
...
| fm-private-lib | absent | 18 cases, both branches of the probe proven on any host, the last two through the Perl capture helper |
