...
| 21 | POSIX mode privacy checks | **FIXED** (issues #4 and #3; decision D6 settled for the helper) | 32 of 151 `bin/*.sh` create or assert mode-700/600 private state across 105 sites, and on a `noacl` mount every one of those checks misfired (row 9, and the `fm-x-mode`, `fm-test-run` and `fm-pr-merge` failures, were all instances). The measurement is sharper than this row first recorded: a `noacl` mount stores no mode at all and `stat` SYNTHESIZES one from the umask of the process doing the READING, `0644 & ~umask` for a file and `0755 & ~umask` for a directory, so one file reads 644 under `umask 022` and 600 under `umask 077` while carrying neither. That is why a probe must watch a mode MOVE rather than ask one question: `chmod 0600` reading back 600 is what a caller that set `umask 077` sees on this mount, and `fm_pr_poll_prepare` sets exactly that. Fixed by `bin/fm-private-lib.sh`, a leaf that probes the filesystem under the path in hand (`chmod 0644`, read, `chmod 0600`, read), caches the verdict per device, keeps today's exact strictness wherever a mode is carried, and records the waiver in `FM_PRIVATE_MODE_UNENFORCEABLE` where it is not. An `acl` fstab mount still works where one exists and is not required. The relaxation is proved on Git Bash only: a mode-dropping filesystem whose synthesized directory mode carries group or other write trips the probe's directory guard, keeps the strict branch and refuses, which the library header records as a known gap rather than handles, and no such filesystem has been measured. The 32 and the 105 count `bin/*.sh` only: `bin/fm-procevent-extension-capture.pl` asserts the same modes in Perl on the extension capture path and is now handed the owner's verdict for each filesystem its own assertions land on, as `--private-modes <device>=enforcing|unenforceable` pairs, rather than measuring one of its own or reading one verdict for paths on different storage, while `bin/fm-extension.mjs` still compares exact modes on that same path and is what is left outside the owner. | issues #4 and #3; `tests/fm-private-lib.test.sh` 21/21, both filesystems staged so both branches run on any host, including the capture helper across the language boundary, the verdict that must not cross one, and the probe's own `chmod` target |
...
That is written about the Stop hook, where the sidecar below is a second proof that can replace the parent; for SessionStart there is no such second proof, and issue #2 later did edit the registrations, using a form that removes the discretion instead of depending on it, since `X; exit 0` pins the hook's status to 0 while exec-ing `X` would hand the harness `X`'s own status, so no correct shell may fold the two together.
...
That slice is issue #6 below ("the cooperation window is a measured deadline"), which replaced the count with a wall-clock deadline sized from the host's own recorded time-to-claim.
...
| 24 | Guarded PR merge | **FIXED** (issue #3) | `bin/fm-pr-merge.sh` insisted on the PR-poll registration `fm_pr_poll_prepare` performs, which asserts mode-600 private files, so every merge on this host answered `error: could not prepare PR poll` and the forge was never called: the guarded path was inert and the captain's merges went through the forge tool. Row 21 owns the fix. The site that actually failed is worth naming, because it is not the one the row predicted: `$id.check.sh` is a copy of an EXECUTABLE template, so it reads back 700 rather than 600 under the `umask 077` the registration sets, while the two plain files read 600 and passed. | issue #3; `tests/fm-pr-merge.test.sh` proves the merge end to end against a staged mount that cannot represent a mode |
...
| 27 | Stop-hook cooperation window | **FIXED** (issue #6) | The synchronous guard waits `SYNC_WAIT_MS / 100` iterations of `sleep 0.1` plus one poll for the auto-arm to claim, which assumes a free poll. Measured here: `wait=0` 3394 ms, `wait=800` 8896 ms, `wait=8000` 52845 ms - about 620 ms per iteration, because `fm_pid_alive` on a native pid falls back to a whole-table `ps -W` (1.2 s). Meanwhile the auto-arm's identity proof alone is 2130 ms + 3009 ms in the severed-hook shape, so the first Stop of a session costs one forced continuation (`TURN WOULD END BLIND`) before the hook can claim. Every later Stop allows in 2.4 s. | issue #6: a wall-clock deadline instead of an iteration count, sized from the home's own recorded time-to-claim; see "Issue #6: the cooperation window is a measured deadline" below |
| 28 | `fm-claude-stop-autoarm` suite | **FIXED** (issue #10; the historical red was 1 of 37) | `install_autoarm_scripts` never copied `bin/fm-proc-lib.sh`, which slice 1 made `fm-wake-lib.sh` source, so the whole suite died on `FM_PROC_UNAME: unbound variable` at case 1 on every platform - the sixth site of slice 6's omission. `tests/fm-turnend-guard.test.sh` had two more, plus two open-coded fakebin symlink loops. With those fixed and two 2.5 s fixture waits raised, the suite runs 36 green and stops at `the superseded owner must exit 0 instead of double-translating: expected exit 0, got 2`, reproduced with the product code stashed. Closed by issue #10: a fixture window, not a product defect (see "Issue #10" under Integration). | closed |
...
Staging the interruption for real exposed the next red in `tests/fm-backlog-handoff.test.sh`: `test_pre_move_crash_does_not_wake_until_move_lands` now gets past its crash fixture on Windows and stops at `unrelated handoff changed the prepared batch's source item`, which is green on Linux.
That red is the fixture's own rather than a finding about what a killed handoff leaves behind (issue #28): after killing the handoff, the fake `tasks-axi` went on to run the real move itself, and wherever the rest of the case outlasts the fake's one-second sleep, as the unrelated handoff does here, that move lands mid-case and removes the source item the assertion expects to survive.
The fake now exits after the kill without moving anything, and `test_move_crash_keeps_wake_pending_for_recovery` still covers a move that lands before the crash.
...
That was row 22's remaining half, and the issue sweep's #2 took a first pass at it: on an MSYS userland whose ancestry names no harness the hook's open goes to the nudge tier, so the banner is gone and the agent is asked for the one run that can take the helm.
The redundant digest was not gone, which is what the sentence here used to claim: the divert removed the hook's futile copy, but the digest the agent then ran by hand is still a digest, and nothing guaranteed it ran at all.
Issue #2's second pass removed the redundancy by making the hook's own digest work, editing the tracked Claude and Cursor registrations so the wrapper is forked rather than exec'd and the walk reaches `claude.exe`.
...
Four fixes finished.
The table has since outgrown that sweep: issue #2's row carries a later second pass, and issue #19's row was added beside it, both landed on `fm/roadmap-functional` rather than in the sweep's own PR.
...
| #2 | 22 | Opus | `bin/fm-sessionstart-run.sh` hands the open to the nudge tier on an MSYS userland whose ancestry names no harness, instead of a digest that cannot take the lock; only the digest-running sources pay for that walk, since `resume`, `reload` and `fork` reach the nudge either way. A second pass closed the row: the tracked Claude and Cursor session-open registrations now end in `; exit 0` instead of exec-ing the wrapper, so on Git Bash the hook keeps a live parent, the hybrid walk crosses it to `claude.exe`, and the hook's own digest can take the lock before the first turn. The divert stays as the fallback for an exec-ing registration, a host without `pwsh`, or a native parent the classifier cannot name | four cases (a severed MSYS ancestry nudges, POSIX still runs the digest, an MSYS transport that keeps its harness parent still re-emits on compact, and an MSYS `resume` delegates without walking the ancestry at all), proven in isolation with three green and the live-harness re-emit case red on this box at the 120 s digest bound rather than on its own logic; the suite still stops at its known OpenCode case 8. The second pass adds one case per registration, each running the tracked command string the way the harness does against a stub that records its own parent and exits 7: red on both axes beforehand (status 7, and the stub's parent is the test process itself, checked separately so neither assertion rides on the other), green after with the status pinned to 0 and the stub's parent the surviving `bash -c` shell |
...
| #19 | 22 | Opus | One session open now resolves its ancestry once. `fm_proc_chain_prime` returns early when the memo already holds a `ppid` row for the requested pid, and `fm_harness_ancestry_pid` and `fm_session_lock_owned_by_self` prime the caller's shell before their own `pids=$(...)`, whose prime was always discarded with the substitution. A diverted open hands its answer to the nudge as `--no-harness-ancestry`, never through the environment, because an exported `FM_PROC_CHAIN_MEMO` is inherited by the digest, by `bin/fm-spawn.sh`, and by every crew harness those start, and `_fm_proc_msys_field` reads the memo before `/proc` or `pwsh` | Three cases, each asserting an exact count the old code produces as a different exact number: three primes in one chain start one `pwsh` rather than three, a resolving MSYS `compact` open pays one walk rather than two, and a diverted MSYS open with a live foreign lock pays one rather than two while printing the same single nudge line. Measured 2026-09-12 against throwaway primaries on this box, before-bin taken from the tests-only commit: a resolving `compact` open went 2 walks to 1 with `--reemit --source compact` unchanged (8 interleaved pairs, wall clock median 11.9 s to 9.4 s), and a severed open with a live foreign lock went 2 walks to 1 and 1 `ps -W` table scan to 0 (5 interleaved pairs, median 9.2 s to 6.7 s), against a reference walk of 1.7 s to 3.6 s from a live Claude Bash-tool shell on a box running three other workers. Every count was identical in every pair; the wall clock has a per-run variance wider than the effect under that load, so the counts are the evidence and the medians only corroborate them |
...
It is the fixture.
The case parks owner A inside a fake arm that sleeps six seconds, fires B (which must defer), makes A's claim stuck-shaped, and fires C, which must release the abandoned claim, take the next generation, arm and translate; the supersession has to land inside what is left of those six seconds.
On WSL Ubuntu the whole suite is 41 of 41 in 22 s, because B and C's work costs milliseconds; here each firing costs seconds (the identity proof, the ledger reads, the fake claude wrapper), and a kept run shows A writing its banner at 22:25:12 and C's arm starting at 22:25:19.
So A was still the unchallenged owner of generation 1 when its arm returned, committed `rewake` and exited 2 exactly as designed, and C then took generation 2 on top of a finished claim.
The assertion read a correct exit 2 as double-translation.
...
Linux: unchanged, 41 of 41 at the shipping head 405cb84 and at the base 5ed6c5e on WSL Ubuntu 24.04 (14 s and 20 s), re-measured after the two review commits that touched the same fixture.
...
### Issue #6: the cooperation window is a measured deadline

The guard's `--claude` cooperation window was `SYNC_WAIT_MS / 100` iterations of `autoarm_owns_recovery` plus a `sleep 0.1`, which assumes the poll is free.
It is not free here: one poll costs about 620 ms, because its identity proofs read `/proc` and `fm_pid_alive` on a native pid falls back to a whole-table `ps -W`.
Measured again on this branch with the unfixed guard, a nominal 3000 ms window spent 23038 ms of waiting, and finding 27's nominal 800 ms spent 5.5 s against an auto-arm that needs about 5.1 s to claim in the severed-hook shape.
So the window was both far longer than its budget and still too short for the thing it was waiting for, and every first Stop of a session paid one forced continuation.
...
A record that only rises needs a ceiling, though, because the measurement is a wall-clock delta rather than a monotonic one: a suspend or an NTP step between the hook's start and its claim lands in it whole, and keeping the maximum would then pin the record at that artifact forever.
...
With `state/.claude-autoarm-claim-ms` = 3000 and a 100 ms budget: one poll unfixed, 6320 ms fixed, which is the record doing its work.
Under a 60 s record with a 2000 ms cap the fixed guard waited 2300 ms, and with a malformed cap 15691 ms, which is the default cap applying rather than the measurement.

The suite does not assert those durations, and the first version of these cases, which did, is worth recording as a measurement of its own.
A whole `--claude` run here is mostly process start and block-path cost; that cost drifts by seconds between runs, and a second block against the same home walks further into the re-block budget, which is slower again.
The same nominal 3000 ms window read as 7307 ms once and 1374 ms another time against a subtracted baseline, so no threshold separates the two timing models reliably.
The cases now publish a claim at a known instant from a background writer and read the guard's exit code, which is immune to host cost: a claim 8 s out misses a 3000 ms window (a counted window ran about 20 s here and honored it), a home that recorded 4000 ms honors a claim 4 s out that the default window misses, the same 60 s record blocks that 8 s claim under a 3000 ms cap and honors it under the default one, and a malformed record neither widens the window nor breaks the deadline.
The review rounds added two more of the same shape, because a zero-padded record or a zero-padded floor or cap reads as octal in `$(( ))` while the test builtin reads it as decimal: an unnormalised 07000 buys half the window it asked for, and an unnormalised 09 fails the expansion and leaves the guard to die on an unbound deadline under `set -u` before it reaches any Stop decision.

Eight new guard cases and three auto-arm cases, each red before the change and green after except one negative control that is marked as such, proven one at a time through a one-case copy of its suite with the `bin/` change reverted under it.
Six of the guard cases and one of the auto-arm cases landed with the slice; the rest came from the review rounds that followed, which added the leading-zero readings of the record and of the window bounds, the monotonic record, and its ceiling.
`tests/fm-turnend-guard.test.sh` runs 43 green here and still stops in the OpenCode plugin's node harness, which is issue #10 and not this slice; with that case and the two Pi extension cases (the same node class, newly visible because they sit behind it) deregistered, the suite reaches its `--claude` tail and runs 74 green, all eight new window cases among them, once `test_hook_runs_fast` is deregistered too - that case sits exactly on its own 3 s bound here and flips either way with this change or without it, which is the wall-clock-budget class this finding names rather than anything this slice did.
`tests/fm-claude-stop-autoarm.test.sh` runs 39 green, three more than the 36 this ledger recorded before, and still stops at finding 28's `the superseded owner must exit 0 instead of double-translating`.
`tests/fm-claude-stop-autoarm-live-e2e.test.sh` self-skips without `FM_CLAUDE_LIVE_E2E=1`, so the tenth assertion that finding 27 owns is not re-measured here.
...
The owner is shell, and the private-state sites are not all shell: `bin/fm-procevent-extension-capture.pl` asserts a 0700 registry, inbox and reservation root and a 0600 result and claim in Perl, in its own process, and refused every one of them on a mount that stores no mode.
It does not get a probe of its own - that would be a second answer to a question this owner exists to answer once - so `bin/fm-procevent.sh` asks the owner and passes the verdict down as `--private-modes <device>=enforcing|unenforceable`, and every comparison in the helper looks its own verdict up by the device it just stat'ed.
Per filesystem, not per invocation: the registry, inbox, published result and reservation root are this home's state, while the claim file resolves through the claim-root override, then the XDG state home, then the home directory, so it can sit on storage with a different answer, and a single verdict would have relaxed a comparison on a volume that carries modes perfectly well.
A device the argument does not name, an entry that does not parse, a value that is not `unenforceable`, and no argument at all are all the strict mode, and only the mode equality ever relaxes: ownership, symlink, type, link-count, size and `O_CREAT|O_EXCL|O_NOFOLLOW` all keep refusing, and files are still created asking for 0600.
`bin/fm-extension.mjs` carries the same exact-mode assertions on the same capture path and is deliberately untouched here, so THE EXTENSION CAPTURE PATH IS NOT END TO END WORKING ON A MOUNT THAT STORES NO MODE.
What the Perl fix changed there is where it stops, not whether it stops, and the new place is worse to be: before, `cmd_start` died at the helper's first directory check and answered `cannot safely stage the extension result` having written nothing, whereas now the helper completes - it publishes `<id>.<n>.result` with its `.adapter` and `.extension` sidecars into the inbox and writes two capture reservation records - and the refusal lands afterwards in the node host, which compares `modeOf(info) !== 0o600` on the captured result, the claim and the capability.
The registration stays armed and that published result is never acknowledged, so a source that keeps producing output accumulates inbox sequences nothing can handle, where the old behaviour failed clean.
Reaching it at all needs a registration made while modes were carried, because the node host also requires an installed package tree at exactly 0555/0444 and such a mount cannot report that either.
The node half is out of scope for this change and is being filed as its own issue: it asserts those read-only package modes as well, which is an immutability claim rather than a privacy one, and decision D6 does not settle that trade.
...
| fm-private-lib | absent | 21 `ok` lines, both branches of the probe proven on any host, the last three through the Perl capture helper |

The base column quotes the wording the base printed; those three mode assertions now name the observed mode instead, so a failure there reads `the captured result is not mode 0600: 644` rather than as a statement that it is private.
The counts are `ok` lines from a run, not a count of case functions: three of the assertion cases share one helper that prints per caller.

Fixture timing budgets surfaced behind the fixed mode gate and were sized to the host rather than enlarged, following issue #8's rule.
`fm-procevent` had two: five fixed half-second waits, and a sibling process that had to outlive a setup slower than its own fixed thirty seconds.
...
`fm-pr-check-security`'s `run_watcher_bounded` had two more, a ten-second kill budget and a one-second per-check timeout, and both now go through `fm_test_seconds`; the one case that needs a check to time out keeps its literal second on its own `run_check` call, and the helper's comment carries the per-run cost that made one second unreliable here (issue #33).
