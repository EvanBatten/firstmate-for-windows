...
| 25 | Watcher identity across a clock step | **FIXED** (issue #5 in the issue sweep; the residual it left closed by issue #17) | `fm_pid_identity`'s `proc-starttime` is `/proc/<pid>/stat` field 22, and its comment says that field is "immune to the wall-clock steps". True on Linux, false here: `/proc/stat`'s `btime` is `now - uptime` RECOMPUTED at every read (measured: `1788065531 - 976306 = 1787089225` against a reported `1787089224`), and field 22 is anchored to that derived origin (`btime + field22/CLK_TCK = 1788065561` against Win32's true creation time `1788065562`). `CLK_TCK` here is **1000**, not 100, so Phase C's 1041-tick drift is 1.04 s of clock adjustment, not 10.4 - an ordinary NTP resync evicts a live watcher and a closed lid is not required. Both values stable to the tick over 25 s with no step. The sweep built the recommendation this row made, in a corrected form the review of that PR forced: `btime + field22/CLK_TCK` is invariant only under a WHOLE-second step, because `btime` is truncated to seconds while field 22 carries milliseconds, so a fractional correction - the ordinary NTP shape - still moves the sum by a second. What shipped is the same origin at full precision, `now - uptime + field22/CLK_TCK`, where `now` is bash's `EPOCHREALTIME` and `uptime` is monotonic; the sweep floored that sum to a whole second and issue #17 removed the floor. The residual that reading left was `/proc/uptime`'s centisecond granularity plus the gap between the two clock reads, persistent per process rather than random per read: a process whose creation instant landed within about 15 ms of a second boundary could be recorded one second apart by two readers for its whole life, so every one of its health checks was a coin toss rather than one unlucky read. **Issue #17 closed it.** The key is now `proc-createtime-ms` and carries the summed milliseconds unfloored, and `fm_pid_identity_equal` in `bin/fm-wake-lib.sh` - whose header, beside `fm_pid_identity`'s, is the single owner of this contract - is the one comparator every identity equality in `bin/` routes through. It accepts a difference of up to `FM_PID_IDENTITY_TOLERANCE_MS` (100, against a measured residual of about 15) on that key alone and only with a byte-identical `cmdline-hex`, and keeps `linux-starttime`, `proc-starttime` and the `ps lstart` dialect byte-exact. That 100 is a bound rather than headroom: the reader brackets its one `/proc/uptime` read with an `EPOCHREALTIME` read on either side, pairs the uptime with their midpoint and retries up to three times when they are more than 10 ms apart, so one reader is off by at most about 16 ms and two by at most about 32, and the residual is four consecutive stalls over 10 ms, which record the least-stalled sample and self-heal at the next arm. The tolerance narrows the pid-reuse window rather than widening it: a reused pid that also relaunches a byte-identical command line compares equal within 200 ms around the original's creation instant, against a full second under the whole-second key and about a millisecond under the raw field 22 before the sweep. The whole-second `proc-createtime` key is retired with no compatibility shim, so a lock written by the previous build mismatches exactly once, at upgrade, and is reclaimed and re-armed - the same one-time cost the move from `proc-starttime` to `proc-createtime` already paid. `bin/fm-teardown.sh`'s `task_process_identity` was the last second reader of raw field 22 and is now a thin wrapper over `fm_pid_identity` comparing through the same comparator; its `lsof` gate and every signalling decision are unchanged, and the compare is strictly stricter than before because `cmdline-hex` now participates, which costs one extra reap pass for a process that `exec`s between capture and signal. `bin/fm-extension.mjs` is deliberately outside this contract: on Linux and macOS it already emits byte-identical strings, and on Windows it runs under native node, cannot read the MSYS `/proc` at all, and reports `process-identity-uncertain`, so unifying it needs a native-node-to-MSYS bridge and is a follow-up. | slice 10 measured it and changed no code; the sweep built it, issue #17 removed the residual |
...
| #5 | 25 | Opus | Superseded by issue #17; ledger row 25 owns the current contract. What landed then: `fm_pid_identity` records `now - uptime + field22/CLK_TCK` on a non-Linux `/proc` under its own `proc-createtime` key, floored once, `now` from bash's own `EPOCHREALTIME` and `CLK_TCK` read once; an unreadable origin keeps the raw field 22 under the `proc-starttime` key this dialect always used, because Cygwin `ps` rejects the portable fallback's `-o` fields and reaching it would mean no identity at all | a fake `/proc` stepped by 3 s and by a fractional 1.25 s yields the same identity, the fractional case being the phase where the btime form breaks; 20 reads of a live pid over 45 s, one string |
