...
Six new guard cases and one auto-arm case, each red before the change and green after except one negative control that is marked as such, proven one at a time through a one-case copy of its suite with `bin/` restored from `origin/windows`.
`tests/fm-turnend-guard.test.sh` runs 43 green here and still stops in the OpenCode plugin's node harness, which is issue #10 and not this slice; with that case and the two Pi extension cases (the same node class, newly visible because they sit behind it) deregistered, the suite reaches its `--claude` tail and runs 72 green, all six new window cases among them, once `test_hook_runs_fast` is deregistered too - that case sits exactly on its own 3 s bound here and flips either way with this change or without it, which is the wall-clock-budget class this finding names rather than anything this slice did.
`tests/fm-claude-stop-autoarm.test.sh` runs 37 green, one more than the 36 this ledger recorded before, and still stops at finding 28's `the superseded owner must exit 0 instead of double-translating`.
...
Linux is unchanged apart from the new cases: on WSL Ubuntu the guard suite runs 70 green at the merge base and 76 green on this branch, and the auto-arm suite 41 then 42, all four runs fully green, with the guard suite going from 34 s to 75 s because the new cases wait for claims that a Linux host would never be slow enough to need.
