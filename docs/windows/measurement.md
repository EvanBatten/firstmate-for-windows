...
| #2 | 22 | Opus | `bin/fm-sessionstart-run.sh` hands the open to the nudge tier on an MSYS userland whose ancestry names no harness, whatever the source, instead of a digest that cannot take the lock | three cases (a severed MSYS ancestry nudges, POSIX still runs the digest, and an MSYS transport that keeps its harness parent still re-emits on compact), each green in isolation; the suite still stops at its known OpenCode case 8 |
