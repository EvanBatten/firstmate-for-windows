...
Ownership is proven by the harness ancestry, and - only where that walk can name no harness at all, which is what MSYS `exec` leaves a Git Bash hook with - by the Stop payload's `session_id` matching the identity `bin/fm-lock.sh` recorded beside the pid in `state/.lock.session`, against a recorded owner that is still a live harness.
