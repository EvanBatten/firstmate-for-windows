...
FM_PROC_ROOT_OVERRIDE=   # alternate /proc root for fm_pid_identity's process-identity reads in fm-wake-lib.sh, which fm-teardown.sh shares, mainly for tests
FM_PROC_NOW_OVERRIDE=    # wall clock in milliseconds since the epoch for fm-wake-lib.sh process identity on a non-Linux /proc, one value or a whitespace-separated sequence consumed one per read with the last repeating, mainly for tests
...
FM_INACTIVE_RECONCILE_BUDGET_SECS=10  # 1..30-second scan deadline counted from process start; the scan ends itself, there is no kill backstop
...
FM_CLAUDE_AUTOARM_SYNC_WAIT_MAX_MS=120000   # cap on how far a recorded time-to-claim may widen that window; an explicitly configured FM_CLAUDE_AUTOARM_SYNC_WAIT_MS above the cap is still honored
FM_CLAUDE_AUTOARM_FIRST_CLAIM_WAIT_MS=15000   # milliseconds a home that has never claimed waits on its first turn end; default 90000 on Git Bash/MSYS; this is not the measured-window cap
...
FM_ARM_CONFIRM_TIMEOUT=10   # seconds fm-watch-arm waits to confirm a fresh watcher before reporting FAILED; default 120 on Git Bash/MSYS
FM_ARM_ATTACH_POLL=0.5  # seconds between checks while fm-watch-arm is attached to an existing healthy watcher cycle
FM_OPENCODE_ARM_READY_TIMEOUT_MS=12000   # milliseconds the OpenCode primary watcher plugin waits for an arm attempt to report started, healthy, wake, or failure; default 140000 on Windows to stay above the MSYS confirm budget
FM_PI_ARM_READY_TIMEOUT_MS=12000   # milliseconds the Pi watcher extension waits for a successor arm to report started or attached; default 140000 on Windows to stay above the MSYS confirm budget
