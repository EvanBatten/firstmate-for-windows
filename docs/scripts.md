...
| `fm-timing-lib.sh`       | Single owner of the repo's millisecond clock and of per-step elapsed-time records for the deferred network stage and for FM_SESSION_START_PROFILE, inert unless a run asks for them |
| `fm-supervision-lib.sh`  | Shared in-flight-work-without-fresh-watcher-beacon predicate                         |
| `fm-ff-lib.sh`           | Shared guarded fast-forward helper for origin pulls and local secondmate syncs       |
| `fm-lock-lib.sh`         | Shared "is this git lock provably abandoned?" proof used by teardown and fleet-sync   |
| `fm-private-lib.sh`      | One owner of "this path must be private": the mode-700 directory, the mode-600 file, and the measured question of whether the filesystem underneath can carry either |
