...
This repository is the Windows port of firstmate and is maintained separately from [upstream](https://github.com/kunchenguid/firstmate).
Contributions here land here.
Nothing is forwarded upstream on your behalf, and opening a PR here is not a submission to that project.

**What belongs here:** anything that makes the distro run correctly on Windows under Git Bash, plus the port's own documentation and tests.
**What belongs upstream:** changes to firstmate's behavior on macOS and Linux, or to its design.
A fix that is genuinely cross-platform is welcome here too, but upstream is where it will help the most people.

One rule up front:

**Human-authored pull requests must be raised through [`no-mistakes`](https://github.com/kunchenguid/no-mistakes).**
We require this to reduce the maintainer's burden of reviewing and merging contributions.

`no-mistakes` puts a local git proxy in front of your real remote.
Pushing through it runs an AI-driven review/test/lint pipeline in an isolated worktree, forwards the push upstream only after every check passes, and opens a clean PR automatically.

A GitHub Actions check (`Require no-mistakes`) enforces this, requiring both the deterministic signature and a parseable structured attestation from no-mistakes v1.46.0 or newer.
...
1. Fork this repo, then clone it or set your local `origin` back to `git@github.com:EvanBatten/firstmate-for-windows.git`.
   Branch from `windows`, which is the default branch and the one PRs target; `main` only mirrors upstream and takes no changes.
2. Create a branch and make your changes.
3. Initialize the gate with your fork as the push target: `no-mistakes init --fork-url git@github.com:<you>/firstmate-for-windows.git` (structured attestation requires **no-mistakes v1.46.0+**; without a fork, plain `no-mistakes init` still works for maintainers with push access).
...
The same file sets `test.instructions`, the gate Test agent's working contract for this fork's Git Bash host, where a whole suite can take twenty minutes; it owns the exact rules there and why a Windows box needs them.
...
Stock macOS Bash 3.2 scans a `$( ... )` body for quotes and backticks even inside a heredoc, so keep them balanced in any heredoc that feeds a command substitution, including apostrophes in comments and prose.
...
Ask a process's parent, group, command name, arguments, or live state through `fm_test_ppid`, `fm_test_pgid`, `fm_test_comm`, `fm_test_args`, and `fm_test_stat` in `tests/lib.sh` rather than spelling `ps -o`, which MSYS `ps` rejects; a standalone fakebin script sources `bin/fm-proc-lib.sh` by absolute path and calls `fm_proc_*` directly.
...
Bound the test's own waiting with `fm_test_wait_until <linux-seconds> <predicate...>` and pass a budget the product reads through `fm_test_seconds <n>` (or `fm_test_tenths <ticks>` for a helper that still counts tenths): both size the Linux budget by the host time scale `tests/lib.sh` measures from this machine's spawn cost, so the happy path returns as soon as its condition holds and only the bound moves on a slow host.
Do not write a tick-counted `while [ "$i" -lt N ]; do ...; sleep 0.1` loop or a bare wall-clock constant for a wait: a spawn here can cost thirty times what it costs on Linux, and such a budget then fails without any defect (docs/windows/measurement.md, issue #8).
When the awaited event can land while `bin/fm-watch.sh` is still starting or in its first poll (a restarted or successor watcher, a signal written just after it takes the lock), budget the wait at the watcher family's 10 s (`fm_test_wait_until 10`, or 100 ticks through `wait_for_exit`), not a multiple of `FM_POLL`: the watcher's bounded startup work outruns a poll-interval budget on a slow host and the spawn-cost scale understates it, so the tighter budget reaps a watcher that is still starting rather than one that went blind, while one that never surfaces still fails when the budget runs out (the rule above `wait_numeric_file` in tests/fm-watch-triage.test.sh).
...
Questions about firstmate itself, rather than the Windows port, belong with [upstream](https://github.com/kunchenguid/firstmate).
